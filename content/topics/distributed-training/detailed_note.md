...
Distributed training splits the job across many GPUs. But "split it up" isn't one technique — there are genuinely different things that can be split (the data, the layers, the weights themselves), and each choice trades off differently against how much the GPUs have to talk to each other, which ties directly back to the interconnects from [Hardware for AI](/topic/hardware-for-ai).
...
**The idea:** even after splitting the model across GPUs, one thing can still blow up memory — **activations**, the intermediate values saved during the forward pass for use in the backward pass. For long sequences (think 32K+ tokens), activation memory can rival the memory used by the parameters themselves.
...
This is usually combined with tensor parallelism rather than used alone — tensor parallelism spreads out *weight* memory, sequence parallelism spreads out *activation* memory, and together they cover the two things that dominate memory usage for very large models with long inputs. A closely related idea for extremely long contexts is **Ring Attention**, which extends this same principle specifically to the attention computation.

**Tech stack:** Megatron-LM's sequence-parallel implementation is the standard reference.

## 5. ZeRO / Fully Sharded Data Parallel (FSDP) — Removing Data Parallelism's Waste
...
Real large-model training runs almost never use just one of the strategies above — they combine them, commonly called **3D parallelism**:
...
**Tech stack:** **Megatron-DeepSpeed** (a combination of NVIDIA's Megatron-LM and Microsoft's DeepSpeed) is the reference framework most large open LLM training runs are built on or inspired by, precisely because it implements all three dimensions together rather than requiring each to be hand-integrated.
...
| Tensor Parallelism | Weights inside a layer | Model too big for one GPU | Every layer, every pass |
| Pipeline Parallelism | Layers across GPUs | Model too big for one GPU | At each stage boundary |
| Sequence Parallelism | The input sequence | Activations too big for one GPU (long context) | Per layer, along the sequence |
...
[Rajbhandari et al.'s ZeRO paper](https://arxiv.org/abs/1910.02054) introduces the memory-sharding stages covered in Section 5, [Shoeybi et al.'s Megatron-LM paper](https://arxiv.org/abs/1909.08053) and its follow-up, [Narayanan et al. on training at GPU-cluster scale](https://arxiv.org/abs/2104.04473), cover tensor and pipeline parallelism as implemented in practice, [Huang et al.'s GPipe paper](https://arxiv.org/abs/1811.06965) introduces the micro-batching scheme behind pipeline parallelism, and the [PyTorch FSDP documentation](https://docs.pytorch.org/docs/stable/fsdp.html) covers PyTorch's native sharding implementation.